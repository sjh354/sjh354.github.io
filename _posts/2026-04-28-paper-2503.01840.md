...
> <p><span>TL;DR</span><span>기존 **Feature Prediction** 의 제약을 제거하고 **Training-time test** 와 **Multi-layer feature fusion** 을 도입하여, 데이터 규모에 따라 추론 성능이 선형적으로 확장되는 **Inference Scaling Law** 를 실현한 무손실 가속 방법론</span></p>
{: .block-headinfo}
...
> ** 엔지니어 노트:** 본 연구는 '데이터 스케일링'의 힘이 단순히 모델의 지능뿐만 아니라 '추론의 물리적 속도'에도 적용될 수 있음을 보여준 매우 실무적인 연구입니다. 특히 **Training-time test** 가 오차 누적을 학습 단계에서 해결한 방식은 구현 관점에서도 매우 우아한 해결책입니다.

---

## 1. 핵심 직관 (Core Intuition)
이 논문의 아이디어를 한 마디로 정의하면 **"중간 과정(Feature)의 정답을 맞히려고 애쓰는 대신, 결과(Token)를 직접 예측하며 스스로의 실수에 적응하도록 훈련하는 것"** 입니다. 기존 모델이 완벽한 '중간 요약본'을 만들려다 데이터가 많아져도 한계에 부딪혔다면, **EAGLE-3** 는 실제 추론 상황과 똑같은 환경에서 반복 훈련하여 데이터가 늘어날수록 더 똑똑하게 다음 단어를 추측합니다.
